...
static void LCD_panel_init(u32 sel)
{
	int ret   = 0;
	int busnum = -1;
//	printf("wanpeng>>>enter LCD_panel_init\n");
	ret = fdt_get_one_gpio("/soc/ncs8801s","ncs8801s_reset",&ncs8801s);
	if (!ret) {
		if (ncs8801s.port) {
			ncs8801s_hd = sunxi_gpio_request(&ncs8801s, 1);
			if (!ncs8801s_hd) {
				printf("wanpeng>>>reuqest gpio for led failed\n");
				return;
			}
			ret = gpio_write_one_pin_value(ncs8801s_hd, 0,"ncs8801s_reset");
			if(ret < 0){
				printf("wanpeng>>>gpio_write_one_pin_value write 0 failed\n");
				return;
			}
			mdelay(50);
			ret = gpio_write_one_pin_value(ncs8801s_hd, 1,"ncs8801s_reset");
			if(ret < 0){
				printf("wanpeng>>>gpio_write_one_pin_value write 1 failed\n");
				return;
			}
		}
	}else{
		printf("wanpeng>>>fdt_get_one_gpio /soc/ncs8801s failed\n");
		return;
	}

	busnum = i2c_get_bus_num();
//	printf("i2c_get_bus_num is busnum=%d\n",busnum);
	if (busnum != SUNXI_VIR_I2C4){
		i2c_set_bus_num(SUNXI_VIR_I2C4);
	}
