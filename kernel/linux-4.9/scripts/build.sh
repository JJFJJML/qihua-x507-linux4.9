...
build_modules()
{
    ...
    build_nand
    build_gpu
    if [ "x${LICHEE_LINUX_DEV}" = "xubuntu18" ] || [ "x${LICHEE_LINUX_DEV}" = "xubuntu20" ]; then
        make modules_install INSTALL_MOD_PATH=${LICHEE_KDIR}/output/ ARCH=$ARCH CROSS_COMPILE=$CROSS_COMPILE
    fi
    if [ "x${LICHEE_LINUX_DEV}" = "xdebian9" ] || [ "x${LICHEE_LINUX_DEV}" = "xdebian10" ]; then
        make modules_install INSTALL_MOD_PATH=${LICHEE_KDIR}/output/ ARCH=$ARCH CROSS_COMPILE=$CROSS_COMPILE
    fi
