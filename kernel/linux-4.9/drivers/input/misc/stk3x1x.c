...
static int stk3x1x_i2c_read_data(struct i2c_client *client, unsigned char command, int length, unsigned char *values)
{
	...
	if (retry >= 5) {
		DEBUG_INFO( "%s: i2c read fail, err=%d\n", __func__, err);
		return -EIO;
	}
	return 0;
}
...
static int stk3x1x_i2c_write_data(struct i2c_client *client, unsigned char command, int length, unsigned char *values)
{
	...
	if (!client)
		...
		DEBUG_INFO( "%s:length %d exceeds 10\n", __func__, length);
		return -EINVAL;
    }
	...
	if (retry >= 5) {
		DEBUG_INFO( "%s: i2c write fail, err=%d\n", __func__, err);
		return -EIO;
	}
	return 0;
}
...
static void stk3x1x_proc_plat_data(struct stk3x1x_data *ps_data, struct stk3x1x_platform_data *plat_data)
{
	...
	if (ps_data->wait_reg < 2) {
		DEBUG_INFO( "%s: wait_reg should be larger than 2, force to write 2\n", __func__);
		ps_data->wait_reg = 2;
	} else if (ps_data->wait_reg > 0xFF) {
		DEBUG_INFO( "%s: wait_reg should be less than 0xFF, force to write 0xFF\n", __func__);
...
static int32_t stk3x1x_check_pid(struct stk3x1x_data *ps_data)
{
	...
	if (err < 0) {
		DEBUG_INFO( "%s: fail, ret=%d\n", __func__, err);
		return err;
	}

	DEBUG_INFO( "%s: PID=0x%x, RID=0x%x\n", __func__, value[0], value[1]);

	if (value[1] == 0xC0)
		DEBUG_INFO( "%s: RID=0xC0!!!!!!!!!!!!!\n", __func__);
		...
		case STK3332_PID:
			return 0;
		case 0x0:
			DEBUG_INFO( "PID=0x0, please make sure the chip is stk3x1x!\n");
			return -2;
		default:
			DEBUG_INFO( "%s: invalid PID(%#x)\n", __func__, value[0]);
			return -1;
	}
...
static int32_t stk3x1x_software_reset(struct stk3x1x_data *ps_data)
{
    ...
    if (r < 0) {
		DEBUG_INFO( "%s: software reset: write i2c error, ret=%d\n", __func__, r);
		return r;
    }
    r = stk3x1x_i2c_smbus_read_byte_data(ps_data->client, STK_WAIT_REG);
    if (w_reg != r) {
		DEBUG_INFO( "%s: software reset: read-back value is not the same\n", __func__);
		return -1;
    }

    r = stk3x1x_i2c_smbus_write_byte_data(ps_data->client, STK_SW_RESET_REG, 0);
    if (r < 0) {
		DEBUG_INFO( "%s: software reset: read error after reset\n", __func__);
		return r;
    }
    msleep(1);
    return 0;
...
static int32_t stk3x1x_set_flag(struct stk3x1x_data *ps_data, uint8_t org_flag_reg, uint8_t clr)
{
	...
	w_flag &= (~clr);
	/*DEBUG_INFO( "%s: org_flag_reg=0x%x, w_flag = 0x%x\n", __func__, org_flag_reg, w_flag);	*/
...
static int32_t stk3x1x_enable_ps(struct stk3x1x_data *ps_data, uint8_t enable, uint8_t validate_reg)
{
	...
	if (validate_reg) {
		...
		if (ret < 0)
			DEBUG_INFO( "stk3x1x_validate_n_handle fail: %d\n", ret);
	}
	...
	if (ret < 0) {
			DEBUG_INFO( "%s: write i2c error, ret=%d\n", __func__, ret);
			return ret;
			}
	w_state_reg = ret;
	...
	if (enable) {
		...
		if (!validate_reg) {
			...
			*/
			...
			DEBUG_INFO( "%s: force report ps input event=1, ps code = %d\n", __func__, reading);
		} else
#endif /* #ifdef STK_CHK_REG */
		{
			msleep(4);
			...
			/*wake_lock_timeout(&ps_data->ps_wakelock, 3*HZ);*/
			reading = stk3x1x_get_ps_reading(ps_data);
			DEBUG_INFO( "%s: ps input event=%d, ps code = %d\n", __func__, near_far_state, reading);
		}
...
*/
			DEBUG_INFO( "%s: //disable_irq(ps_data->irq);\n", __func__);
...
static int32_t stk3x1x_get_als_reading(struct stk3x1x_data *ps_data)
{
	...
	if (ret < 0) {
		DEBUG_INFO( "%s fail, ret=0x%x", __func__, ret);
		return ret;
	}
	...
	irs_alsctrl = (ps_data->alsctrl_reg & 0x0F) - 2;
		...
		case 9:
			*slp_time = 96;
			break;
		default:
			DEBUG_INFO( "%s: unknown ALS IT=0x%x\n", __func__, irs_alsctrl);
			ret = -EINVAL;
			return ret;
	}
...
static int32_t stk3x1x_get_ir_reading(struct stk3x1x_data *ps_data)
{
	...
	if (retry == 10) {
		DEBUG_INFO( "%s: ir data is not ready for 300ms\n", __func__);
		ret = -EINVAL;
		goto irs_err_i2c_rw;
	}
...
static int stk3x1x_chk_reg_valid(struct stk3x1x_data *ps_data)
{
	...
	for (cnt=0; cnt<9; cnt++) {
		...
		if (value[cnt] < 0) {
			DEBUG_INFO( "%s fail, ret=%d", __func__, value[cnt]);
			return value[cnt];
		}
	}
	*/
	...
	if (value[0] != ps_data->psctrl_reg) {
		DEBUG_INFO( "%s: invalid reg 0x01=0x%2x\n", __func__, value[0]);
		return 0xFF;
	}
	if (value[1] != ps_data->alsctrl_reg) {
		DEBUG_INFO( "%s: invalid reg 0x02=0x%2x\n", __func__, value[1]);
		return 0xFF;
	}
	if (value[2] != ps_data->ledctrl_reg) {
		DEBUG_INFO( "%s: invalid reg 0x03=0x%2x\n", __func__, value[2]);
		return 0xFF;
	}
	if (value[3] != ps_data->int_reg) {
		DEBUG_INFO( "%s: invalid reg 0x04=0x%2x\n", __func__, value[3]);
		return 0xFF;
	}
	if (value[4] != ps_data->wait_reg) {
		DEBUG_INFO( "%s: invalid reg 0x05=0x%2x\n", __func__, value[4]);
		return 0xFF;
	}
	if (value[5] != ((ps_data->ps_thd_h & 0xFF00) >> 8)) {
		DEBUG_INFO( "%s: invalid reg 0x06=0x%2x\n", __func__, value[5]);
		return 0xFF;
	}
	if (value[6] != (ps_data->ps_thd_h & 0x00FF)) {
		DEBUG_INFO( "%s: invalid reg 0x07=0x%2x\n", __func__, value[6]);
		return 0xFF;
	}
	if (value[7] != ((ps_data->ps_thd_l & 0xFF00) >> 8)) {
		DEBUG_INFO( "%s: invalid reg 0x08=0x%2x\n", __func__, value[7]);
		return 0xFF;
	}
	if (value[8] != (ps_data->ps_thd_l & 0x00FF)) {
		DEBUG_INFO( "%s: invalid reg 0x09=0x%2x\n", __func__, value[8]);
		return 0xFF;
	}
...
static int stk3x1x_validate_n_handle(struct i2c_client *client)
{
	...
	if (err < 0) {
		DEBUG_INFO( "stk3x1x_chk_reg_valid fail: %d\n", err);
		return err;
	}

	if (err == 0xFF) {
		DEBUG_INFO( "%s: Re-init chip\n", __func__);
...
static ssize_t stk_als_enable_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	if (enable != ret)
		DEBUG_INFO( "%s: driver and sensor mismatch! driver_enable=0x%x, sensor_enable=%x\n", __func__, enable, ret);
...
static ssize_t stk_als_enable_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (sysfs_streq(buf, "1"))
		...
		DEBUG_INFO( "%s, invalid value %d\n", __func__, *buf);
		return -EINVAL;
	}
		DEBUG_INFO( "%s: Enable ALS : %d\n", __func__, en);
		mutex_lock(&ps_data->io_lock);
...
static ssize_t stk_als_delay_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (ret < 0) {
		DEBUG_INFO( "%s:kstrtoull failed, ret=0x%x\n", __func__, ret);
		return ret;
	}
#ifdef STK_DEBUG_PRINTF
	DEBUG_INFO( "%s: set als poll delay=%lld\n", __func__, value);
#endif
    value = value * 1000 * 1000;
	if (value < MIN_ALS_POLL_DELAY_NS) {
		DEBUG_INFO( "%s: delay is too small\n", __func__);
...
static ssize_t stk_als_firlen_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (value > MAX_FIR_LEN) {
		DEBUG_INFO( "%s: firlen exceed maximum filter length\n", __func__);
	} else if (value < 1) {
...
#ifdef STK_DEBUG_PRINTF
	DEBUG_INFO( "%s: set ps poll delay=%lld\n", __func__, value);
#endif
    value = value * 1000 * 1000;
	if (value < MIN_ALS_POLL_DELAY_NS) {
		DEBUG_INFO( "%s: delay is too small\n", __func__);
...
static ssize_t stk_ps_offset_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (offset > 65535) {
		DEBUG_INFO( "%s: invalid value, offset=%ld\n", __func__, offset);
		return -EINVAL;
	}
...
static ssize_t stk_ps_distance_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	/*support wake lock for ps*/
	/*wake_lock_timeout(&ps_data->ps_wakelock, 3*HZ);*/
	DEBUG_INFO( "%s: ps input event %d cm\n", __func__, dist);
	return scnprintf(buf, PAGE_SIZE, "%d\n", dist);
}
...
static ssize_t stk_ps_distance_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
	...
	/*support wake lock for ps*/
	/*wake_lock_timeout(&ps_data->ps_wakelock, 3*HZ);*/
	DEBUG_INFO( "%s: ps input event %ld cm\n", __func__, value);
	return size;
}
...
static ssize_t stk_ps_code_thd_l_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	if (ps_thd_l1_reg < 0) {
		DEBUG_INFO( "%s fail, err=0x%x", __func__, ps_thd_l1_reg);
		return -EINVAL;
	}
	ps_thd_l2_reg = stk3x1x_i2c_smbus_read_byte_data(ps_data->client, STK_THDL2_PS_REG);
	if (ps_thd_l2_reg < 0) {
		DEBUG_INFO( "%s fail, err=0x%x", __func__, ps_thd_l2_reg);
		return -EINVAL;
	}
...
static ssize_t stk_ps_code_thd_h_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	if (ps_thd_h1_reg < 0) {
		DEBUG_INFO( "%s fail, err=0x%x", __func__, ps_thd_h1_reg);
		return -EINVAL;
	}
	ps_thd_h2_reg = stk3x1x_i2c_smbus_read_byte_data(ps_data->client, STK_THDH2_PS_REG);
	if (ps_thd_h2_reg < 0) {
		DEBUG_INFO( "%s fail, err=0x%x", __func__, ps_thd_h2_reg);
		return -EINVAL;
	}
...
static ssize_t stk_als_lux_thd_l_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    if (als_thd_l0_reg < 0) {
		DEBUG_INFO( "%s fail, err=0x%x", __func__, als_thd_l0_reg);
		return -EINVAL;
	}
	if (als_thd_l1_reg < 0) {
		DEBUG_INFO( "%s fail, err=0x%x", __func__, als_thd_l1_reg);
		return -EINVAL;
	}
...
static ssize_t stk_als_lux_thd_h_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    if (als_thd_h0_reg < 0) {
		DEBUG_INFO( "%s fail, err=0x%x", __func__, als_thd_h0_reg);
		return -EINVAL;
	}
	if (als_thd_h1_reg < 0) {
		DEBUG_INFO( "%s fail, err=0x%x", __func__, als_thd_h1_reg);
		return -EINVAL;
	}
...
static ssize_t stk_all_reg_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	for (cnt = 0; cnt < 0x20; cnt++) {
		...
		if (ps_reg[cnt] < 0) {
			DEBUG_INFO( "%s fail, ret=%d", __func__, ps_reg[cnt]);
			return -EINVAL;
		} else {
			DEBUG_INFO( "reg[0x%2X]=0x%2X\n", cnt, ps_reg[cnt]);
...
static ssize_t stk_ps_cali_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	DEBUG_INFO("%s: psi_set=%d, psa=%d,psi=%d, word_data=%d\n", __func__,
...
static int stk_ps_tune_zero_val(struct stk3x1x_data *ps_data)
{
	...
	if (mode == 0x30)
		...
		DEBUG_INFO( "%s: unsupported PS_IT(0x%x)\n", __func__, mode);
		return -1;
	}

	if (word_data > lii) {
		DEBUG_INFO( "%s: word_data=%d, lii=%d\n", __func__, word_data, lii);
		return 0xFFFF;
	}
	return 0;
}
...
static int stk_ps_tune_zero_func_fae(struct stk3x1x_data *ps_data)
{
	...
	if (!(ret&STK_FLG_PSDR_MASK)) {
		/*DEBUG_INFO( "%s: ps data is not ready yet\n", __func__);*/
		return 0;
	}
	...
	if (ret == 0) {
		...
		word_data = (value[0]<<8) | value[1];
		/*DEBUG_INFO( "%s: word_data=%d\n", __func__, word_data);*/

		if (word_data == 0) {
			/*DEBUG_INFO( "%s: incorrect word data (0)\n", __func__);*/
			return 0xFFFF;
		}

		if (word_data > ps_data->psa) {
			ps_data->psa = word_data;
			DEBUG_INFO( "%s: update psa: psa=%d,psi=%d\n", __func__, ps_data->psa, ps_data->psi);
		}
		if (word_data < ps_data->psi) {
			ps_data->psi = word_data;
			DEBUG_INFO( "%s: update psi: psa=%d,psi=%d\n", __func__, ps_data->psa, ps_data->psi);
		}
	}
	...
	if (diff > STK_MAX_MIN_DIFF) {
		...
		stk3x1x_set_ps_thd_l(ps_data, ps_data->ps_thd_l);
#ifdef STK_DEBUG_PRINTF
	DEBUG_INFO( "%s: FAE tune0 psa-psi(%d) > STK_DIFF found\n", __func__, diff);
#endif
	hrtimer_cancel(&ps_data->ps_tune0_timer);
	}
...
static void stk_ps_poll_work_func(struct work_struct *work)
{
	...
	reading = stk3x1x_get_ps_reading(ps_data);
	ps_data->ps_distance_last = near_far_state;
	//DEBUG_INFO("1 proximity data = %d,reading=%d\n", near_far_state,reading);
	input_report_abs(ps_data->ps_input_dev, ABS_DISTANCE, near_far_state);
	input_sync(ps_data->ps_input_dev);
	/*support wake lock for ps*/
	/*wake_lock_timeout(&ps_data->ps_wakelock, 3*HZ);*/
...
#ifdef STK_DEBUG_PRINTF
	DEBUG_INFO("%s: ps input event %d cm, ps code = %d\n", __func__, near_far_state, reading);
#endif
...
static void stk_work_func(struct work_struct *work)
{
	...
	if (org_flag_reg & STK_FLG_ALSINT_MASK) {
		...
		reading = stk3x1x_get_als_reading(ps_data);
		if (reading < 0) {
			DEBUG_INFO( "%s: stk3x1x_get_als_reading fail, ret=%d", __func__, reading);
			goto err_i2c_rw;
		}
#ifndef CONFIG_STK_PS_ALS_USE_CHANGE_THRESHOLD
		...
		if (ps_data->ir_code) {
			...
			DEBUG_INFO( "%s: als=%d, ir=%d, als_correct_factor=%d", __func__, reading, ps_data->ir_code, ps_data->als_correct_factor);
			ps_data->ir_code = 0;
		}
		...
		input_sync(ps_data->als_input_dev);
#ifdef STK_DEBUG_PRINTF
		DEBUG_INFO( "%s: als input event %d lux\n", __func__, ps_data->als_lux_last);
#endif
    }
    if (org_flag_reg & STK_FLG_PSINT_MASK) {
		...
		/*if (near_far_state) {
			DEBUG_INFO( "%s:object leave before report power key event\n", __func__);
			input_report_key(ps_data->ps_input_dev, KEY_PROX_SLEEP, 1);*/
			input_sync(ps_data->ps_input_dev);
			msleep(100);
			/*input_report_key(ps_data->ps_input_dev, KEY_PROX_SLEEP, 0);*/
			input_sync(ps_data->ps_input_dev);
			/*DEBUG_INFO( "%s: object leave after report power key event\n", __func__);
			}*/
		if (!near_far_state) {
			/*DEBUG_INFO( "%s: object come before report power key event\n", __func__);
			input_report_key(ps_data->ps_input_dev, KEY_PROX_WAKE, 1);*/
			input_sync(ps_data->ps_input_dev);
			msleep(100);
			/*input_report_key(ps_data->ps_input_dev, KEY_PROX_WAKE, 0);*/
			input_sync(ps_data->ps_input_dev);
			/*DEBUG_INFO( "%s: object come after report power key event\n", __func__);*/
		}
	}
...
static irqreturn_t stk_oss_irq_handler(int irq, void *data)
{
	...
	/*DEBUG_INFO("aw==== in the stk_oss_irq_handle:0x%x\n", readl(PLDATA_INTERRUPT_REG_VADDR));*/
...
static int stk3x1x_setup_irq(struct i2c_client *client)
{
	...
	DEBUG_INFO("aw==== %s:light sensor irq_number= %d\n", __func__,
	...
	if (0 != ls_sensor_info.int_number) {
		...
		if (err) {
			DEBUG_INFO("Failed to request gpio irq \n");
			return err;
		}
	}
	err = 0;
...
static int stk3x1x_suspend(struct device *dev, pm_message_t mesg)
{
	...
	if (NORMAL_STANDBY == standby_type) {
		...
		if (check_scene_locked(SCENE_TALKING_STANDBY) == 0) {
			DEBUG_INFO("lradc-key: talking standby, enable wakeup source lradc!!\n");
	...
	/*wake_lock(&ps_data->ps_nosuspend_wl);*/
		...
		DEBUG_INFO("check_scene_locked = %d\n", is_talking_standby);
		if (is_talking_standby != 0)
#endif
       {
			stk3x1x_enable_ps(ps_data, 0, 1);
			ps_data->re_enabled_ps = true;
			DEBUG_INFO("aw==== in the re enable ps \n");
       }
#else
		DEBUG_INFO("aw==== suspend \n");


		if (SUPER_STANDBY == standby_type) {
			err = enable_wakeup_src(CPUS_GPIO_SRC, 0);
			if (err)
				DEBUG_INFO( "%s: set_irq_wake(%d) failed, err=(%d)\n", __func__, ps_data->irq, err);
		} else {
			DEBUG_INFO( "%s: not support wakeup source", __func__);
		}
#endif
	}
	mutex_unlock(&ps_data->io_lock);
	...
	if (ls_sensor_info.sensor_power_ldo != NULL) {
		...
		if (err)
			DEBUG_INFO("stk power down failed\n");
	}
...
static int stk3x1x_resume(struct device *dev)
{
	...
	/* process for super standby */
		...
		if (check_scene_locked(SCENE_TALKING_STANDBY) != 0) {
			...
			DEBUG_INFO("aw=== ps-key: resume from talking standby!!\n");
		}
	}
	return 0;
	#endif
	if (ls_sensor_info.sensor_power_ldo != NULL) {
		...
		if (err)
			DEBUG_INFO("stk power on failed\n");
		msleep(100);
		}
	...
	/*wake_unlock(&ps_data->ps_nosuspend_wl);*/
		...
		if (SUPER_STANDBY == standby_type) {
			...
			if (err)
				DEBUG_INFO( "%s: disable_irq_wake(%d) failed, err=(%d)\n", __func__, ps_data->irq, err);
		}
#endif
	...
	org_flag_reg = stk3x1x_get_flag(ps_data);
	if (org_flag_reg & STK_FLG_PSINT_MASK) {
		DEBUG_INFO( "%s:before stk3x1x_set_flag ,org_flag_reg = 0x%X\n", __func__, org_flag_reg);
		disable_flag |= STK_FLG_PSINT_MASK;
		near_far_state = (org_flag_reg & STK_FLG_NF_MASK) ? 1 : 0;
	    reading = stk3x1x_get_ps_reading(ps_data);
		DEBUG_INFO( "%s: ps input event=%d, ps code = %d\n", __func__, near_far_state, reading);
		...
		/*if (!near_far_state) {
		DEBUG_INFO( "%s: object come in resume before report power key event\n", __func__);
		input_report_key(ps_data->ps_input_dev, KEY_PROX_WAKE, 1);*/
		input_sync(ps_data->ps_input_dev);
		msleep(100);
		/*input_report_key(ps_data->ps_input_dev, KEY_PROX_WAKE, 0);*/
		input_sync(ps_data->ps_input_dev);
		DEBUG_INFO( "%s: after report power key event\n", __func__);	/*
		}	*/
	}
	return 0;
}
...
*/
		...
		if (!gUseDetect && startup() != 0)
			return -1;
#endif
		DEBUG_INFO( "%s: driver version = %s\n", __func__, DRIVER_VERSION);

		if (!i2c_check_functionality(client->adapter, I2C_FUNC_I2C)) {
			DEBUG_INFO( "%s: No Support for I2C_FUNC_I2C\n", __func__);
			return -ENODEV;
			}

		ps_data = kzalloc(sizeof(struct stk3x1x_data), GFP_KERNEL);
		if (!ps_data) {
			DEBUG_INFO( "%s: failed to allocate stk3x1x_data\n", __func__);
			return -ENOMEM;
			}
...
/*	wake_lock_init(&ps_data->ps_wakelock,WAKE_LOCK_SUSPEND, "stk_input_wakelock");
	...
	wake_lock_init(&ps_data->ps_nosuspend_wl,WAKE_LOCK_SUSPEND, "stk_nosuspend_wakelock");
		...
		if (ps_data->als_transmittance == 0) {
			DEBUG_INFO( "%s: Please set als_transmittance in platform data\n", __func__);
			goto err_als_input_allocate;
			}

	ps_data->als_input_dev = input_allocate_device();
	if (ps_data->als_input_dev == NULL) {
		DEBUG_INFO( "%s: could not allocate als device\n", __func__);
		err = -ENOMEM;
		goto err_als_input_allocate;
		}
	ps_data->ps_input_dev = input_allocate_device();
	if (ps_data->ps_input_dev == NULL) {
		DEBUG_INFO( "%s: could not allocate ps device\n", __func__);
		err = -ENOMEM;
...
{
	int ret = 0;
	DEBUG_INFO("%s:light sensor driver init\n", __func__);

	if (input_sensor_startup(&(ls_sensor_info.input_type))) {
		DEBUG_INFO("%s: ls_fetch_sysconfig_para err.\n", __func__);
		return -1;
	} else {
		ret = input_sensor_init(&(ls_sensor_info.input_type));
		if (0 != ret) {
			DEBUG_INFO("%s:ls_init_platform_resource err. \n", __func__);
		}
	}
	if (ls_sensor_info.sensor_used == 0) {
		DEBUG_INFO("*** ls_used set to 0 !\n");
		DEBUG_INFO("*** if use light_sensor,please put the sys_config.fex ls_used set to 1. \n");
		return -1;
	}
	return 0;
}
