...
export LICHEE_UBUNTU18_DIR=${LICHEE_TOP_DIR}/release/ubuntu18
export LICHEE_UBUNTU20_DIR=${LICHEE_TOP_DIR}/release/ubuntu20
export LICHEE_DEBIAN9_DIR=${LICHEE_TOP_DIR}/release/debian9
export LICHEE_DEBIAN10_DIR=${LICHEE_TOP_DIR}/release/debian10
export LICHEE_BUILDROOT_DIR=${LICHEE_TOP_DIR}/release/buildroot/buildroot-2018.08
...
function select_linux_development()
{
	...
	[ -d $LICHEE_UBUNTU18_DIR ] && \
	val_list="$val_list ubuntu18"

	[ -d $LICHEE_UBUNTU20_DIR ] && \
	val_list="$val_list ubuntu20"

	[ -d $LICHEE_DEBIAN9_DIR ] && \
	val_list="$val_list debian9"

	[ -d $LICHEE_DEBIAN10_DIR ] && \
	val_list="$val_list debian10"

	[ -d $LICHEE_BUILDROOT_DIR ] && \
...
function prepare_ubuntu18_toolchain()
{
	local ARCH="arm";
	local GCC="";
	local GCC_PREFIX="";
	...
	mk_info "Prepare ubuntu toolchain ..."
	tooldir=${LICHEE_OUT_DIR}/gcc-linaro-5.3.1-2016.05/dragonboard/gcc-arm
...
function prepare_ubuntu20_toolchain()
{
	local ARCH="arm";
	local GCC="";
	local GCC_PREFIX="";
...
function prepare_debian9_toolchain()
{
	local ARCH="arm";
	local GCC="";
	local GCC_PREFIX="";
	...
	mk_info "Prepare debian toolchain ..."
	tooldir=${LICHEE_OUT_DIR}/gcc-linaro-5.3.1-2016.05/dragonboard/gcc-arm
...
{
	if  [ "x$LICHEE_PLATFORM" == "xlinux" ] && \
		[ "x$LICHEE_LINUX_DEV" == "xubuntu18" ] && \
		[ -n "$(ls $LICHEE_PLAT_OUT)" ]; then
		mk_info "clean ubuntu18 ..."

		local script_dir="${LICHEE_UBUNTU18_DIR}/common/scripts/"

		local clean_script="clean.sh"
		(cd ${script_dir} && [ -x ${clean_script} ] && ./${clean_script})

		mk_info "clean ubuntu18 OK."
	fi
}
...
{
	if  [ "x$LICHEE_PLATFORM" == "xlinux" ] && \
		[ "x$LICHEE_LINUX_DEV" == "xubuntu20" ] && \
		[ -n "$(ls $LICHEE_PLAT_OUT)" ]; then
		mk_info "clean ubuntu20 ..."

		local script_dir="${LICHEE_UBUNTU20_DIR}/common/scripts/"

		local clean_script="clean.sh"
		(cd ${script_dir} && [ -x ${clean_script} ] && ./${clean_script})

		mk_info "clean ubuntu20 OK."
	fi
}
...
{
	if  [ "x$LICHEE_PLATFORM" == "xlinux" ] && \
		[ "x$LICHEE_LINUX_DEV" == "xdebian9" ] && \
		[ -n "$(ls $LICHEE_PLAT_OUT)" ]; then
		mk_info "clean debian9 ..."

		local script_dir="${LICHEE_DEBIAN9_DIR}/common/scripts/"

		local clean_script="clean.sh"
		(cd ${script_dir} && [ -x ${clean_script} ] && ./${clean_script})

		mk_info "clean debian9 OK."
	fi
}
...
{
	if  [ "x$LICHEE_PLATFORM" == "xlinux" ] && \
		[ "x$LICHEE_LINUX_DEV" == "xdebian10" ] && \
		[ -n "$(ls $LICHEE_PLAT_OUT)" ]; then
		mk_info "clean debian10 ..."

		local script_dir="${LICHEE_DEBIAN10_DIR}/common/scripts/"

		local clean_script="clean.sh"
		(cd ${script_dir} && [ -x ${clean_script} ] && ./${clean_script})

		mk_info "clean debian10 OK."
	fi
}
...
{
	if  [ "x$LICHEE_PLATFORM" == "xlinux" ] && \
		[ "x$LICHEE_LINUX_DEV" == "xbuildroot" ] && \
		[ -n "$(ls $LICHEE_PLAT_OUT)" ]; then
		mk_info "clean buildroot ..."

		local script_dir="$LICHEE_BUILDROOT_DIR}/common/scripts/"
	...
	if [ ${LICHEE_PLATFORM} = "linux" ] ; then
		case ${LICHEE_LINUX_DEV} in
			...
			dragonboard)
				...
				;;
			ubuntu18)
				prepare_ubuntu18_toolchain
				if [ -d ${LICHEE_UBUNTU18_DIR} ]; then
					echo "Regenerating ubuntu18 Rootfs..."
					(
					cd ${LICHEE_UBUNTU18_DIR}; \
						if [ ! -d "./ubuntu18" ]; then \
							echo "extract ubuntu18-lxde.tar.gz"; \
							tar zxf ubuntu18-lxde.tar.gz; \
						fi
					)
					mkdir -p ${LICHEE_UBUNTU18_DIR}/ubuntu18/lib/modules
					rm -rf ${LICHEE_UBUNTU18_DIR}/ubuntu18/lib/modules/*
					cp -rf ${LICHEE_KERN_DIR}/output/lib/modules/* \
						${LICHEE_UBUNTU18_DIR}/ubuntu18/lib/modules/
					(cd ${LICHEE_UBUNTU18_DIR}/common/scripts; ./build.sh)
					[  $? -ne 0 ] && mk_error "build rootfs Failed" && return 1
					cp ${LICHEE_UBUNTU18_DIR}/rootfs.ext4 ${LICHEE_PLAT_OUT}
				else
					mk_error "no ${LICHEE_PLATFORM} in lichee,please git clone it under lichee"
					exit 1
				fi
				;;
			ubuntu20)
				prepare_ubuntu20_toolchain
				if [ -d ${LICHEE_UBUNTU20_DIR} ]; then
					echo "Regenerating ubuntu20 Rootfs..."
					(
					cd ${LICHEE_UBUNTU20_DIR}; \
						if [ ! -d "./ubuntu20" ]; then \
							echo "extract ubuntu20-core.tar.gz"; \
							tar zxf ubuntu20-core.tar.gz; \
						fi
					)
					mkdir -p ${LICHEE_UBUNTU20_DIR}/ubuntu20/lib/modules
					rm -rf ${LICHEE_UBUNTU20_DIR}/ubuntu20/lib/modules/*
					cp -rf ${LICHEE_KERN_DIR}/output/lib/modules/* \
						${LICHEE_UBUNTU20_DIR}/ubuntu20/lib/modules/
					(cd ${LICHEE_UBUNTU20_DIR}/common/scripts; ./build.sh)
					[  $? -ne 0 ] && mk_error "build rootfs Failed" && return 1
					cp ${LICHEE_UBUNTU20_DIR}/rootfs.ext4 ${LICHEE_PLAT_OUT}
				else
					mk_error "no ${LICHEE_PLATFORM} in lichee,please git clone it under lichee"
					exit 1
				fi
				;;
			debian9)
				prepare_debian9_toolchain
				if [ -d ${LICHEE_DEBIAN9_DIR} ]; then
					echo "Regenerating debian9 Rootfs..."
					(
					cd ${LICHEE_DEBIAN9_DIR}; \
						if [ ! -d "./debian9" ]; then \
							echo "extract debian9-lxde.tar.gz"; \
							tar zxf debian9-lxde.tar.gz; \
						fi
					)
					mkdir -p ${LICHEE_DEBIAN9_DIR}/debian9/lib/modules
					rm -rf ${LICHEE_DEBIAN9_DIR}/debian9/lib/modules/*
					cp -rf ${LICHEE_KERN_DIR}/output/lib/modules/* \
						${LICHEE_DEBIAN9_DIR}/debian9/lib/modules/
					(cd ${LICHEE_DEBIAN9_DIR}/common/scripts; ./build.sh)
					[  $? -ne 0 ] && mk_error "build rootfs Failed" && return 1
					cp ${LICHEE_DEBIAN9_DIR}/rootfs.ext4 ${LICHEE_PLAT_OUT}
				else
					mk_error "no ${LICHEE_PLATFORM} in lichee,please git clone it under lichee"
					exit 1
				fi
			...
			debian10)
				prepare_debian9_toolchain
				if [ -d ${LICHEE_DEBIAN10_DIR} ]; then
					echo "Regenerating debian10 Rootfs..."
					(
					cd ${LICHEE_DEBIAN10_DIR}; \
						if [ ! -d "./debian10" ]; then \
							echo "extract debian10-lxde.tar.gz"; \
							tar zxf debian10-lxde.tar.gz; \
						fi
					)
					mkdir -p ${LICHEE_DEBIAN10_DIR}/debian10/lib/modules
					rm -rf ${LICHEE_DEBIAN10_DIR}/debian10/lib/modules/*
					cp -rf ${LICHEE_KERN_DIR}/output/lib/modules/* \
						${LICHEE_DEBIAN10_DIR}/debian10/lib/modules/
					(cd ${LICHEE_DEBIAN10_DIR}/common/scripts; ./build.sh)
					[  $? -ne 0 ] && mk_error "build rootfs Failed" && return 1
					cp ${LICHEE_DEBIAN10_DIR}/rootfs.ext4 ${LICHEE_PLAT_OUT}
				else
					mk_error "no ${LICHEE_PLATFORM} in lichee,please git clone it under lichee"
					exit 1
				fi
				;;
			buildroot)
				if [ -d ${LICHEE_BUILDROOT_DIR} ]; then
					echo "Regenerating buildroot Rootfs..."
					(
					cd ${LICHEE_BUILDROOT_DIR}; \
						make -j4 ;
					)
					mkdir -p ${LICHEE_BUILDROOT_DIR}/output/target/lib/modules
					rm -rf ${LICHEE_BUILDROOT_DIR}/output/target/lib/modules/*
					cp -rf ${LICHEE_KERN_DIR}/output/lib/modules/* \
						${LICHEE_BUILDROOT_DIR}/output/target/lib/modules/
					(cd ${LICHEE_BUILDROOT_DIR}/common/scripts; ./build.sh)
					[  $? -ne 0 ] && mk_error "build rootfs Failed" && return 1
					mv ${LICHEE_BUILDROOT_DIR}/output/rootfs.ext4 ${LICHEE_PLAT_OUT}
				else
					mk_error "no ${LICHEE_PLATFORM} in lichee,please git clone it under lichee"
					exit 1
				fi
