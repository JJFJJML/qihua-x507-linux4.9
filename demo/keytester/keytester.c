...
int main(int argc, char *argv[])
{
	...
	memcpy(name, "/dev/input/event1", 17);
    }
    ...
    while (1) {
        ...
        for (i = 0; i < rd / sizeof(struct input_event); i++) {
            if (evt[i].type == EV_SYN) {
                ...
                if (pre_key[0]) {
                    snprintf(buf, sizeof(buf), "%s, 0x%2x", 
                            names[evt[i].type] ? (names[evt[i].type][evt[i].code & 0xff] ? names[evt[i].type][evt[i].code & 0xff] : "?") : "?", 
                            evt[i].value & 0xffffff);
                }
                else {
